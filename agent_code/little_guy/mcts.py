...
class TreeNode:
    def __init__(
        self, parent, action, game_state, prior=0, reward=0, collect_coin=False
    ):
        self.parent = parent
        self.action = action
...
class MCTS:
    ...
    def ucb(self, parent, action):
        ...
        ) + self.exploration_constant * np.sqrt(
            np.log(parent.visits) / (child.visits if child.visits != 0 else 0.001)
        )
    ...
    def expand(self, node):
        ...
        for action in ACTIONS:
            if self.state_explorer.is_action_valid(game_state, action):
                new_state, reward, collect_coin = self.state_explorer.step(
                    game_state, action
                )
                ...
                node.children[action] = TreeNode(
                    ...
                    action,
                    new_state,
                    prior=policy[0][ACTIONS.index(action)],
                    reward=reward,
